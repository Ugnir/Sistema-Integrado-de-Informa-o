...
$sql=$pdo->prepare("SELECT atv.num_om,atv.Tare,atv.textoTarefa,atv.gerencial,atv.status,atv.dataAtividade,atv.local,atv.requisitante,atv.numpt,c.* 
...
$excel[]=[
    ...
    "local"=> $planilha["local"],
    "requisitante"=> $planilha["requisitante"],
    "numpt"=> $planilha["numpt"],
  ];
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1', 'Localizacao');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1', 'PT');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1', 'Requisitante');
...
foreach ($excel as $row){
     ...
     $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$linha,$row['local']);
     $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$linha,$row['numpt']);
     $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$linha,$row['requisitante']);
     $linha++;
}
