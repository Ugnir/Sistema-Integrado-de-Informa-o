       <?php 
        $contar=$pdo->prepare('SELECT local,COUNT(local) as contagem FROM atividades atv WHERE 1=1 '.$where.' GROUP BY local HAVING COUNT(local)>1 ORDER BY COUNT(local) DESC');
        $contar->execute();
        $fetch=$contar->fetchall(PDO::FETCH_ASSOC);
        $RowCount = $contar->rowCount();
        ?> 
            <div class="modal fade" id="contagemServIndex">
              <form method="post">
